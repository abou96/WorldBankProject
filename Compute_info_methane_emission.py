...
def is_missing(df, year):
	ismiss = df.loc[df['year'] == year, 'EnergieMethane(ktco2)'].values[0]
	return np.isnan(ismiss)

@dataclass
class CountryEmission : 

    def getMethaneValue(self, CountryAlpha2Code, year) -> pd.DataFrame :
    	...
    	df_country = df_country[['year', 'EnergieMethane(ktco2)']]


    	isMissing = is_missing(df_country, year)

    	if not isMissing :
    		dico['EmissionMethaneValue'] = df_country.loc[df_country['year'] == year, 'EnergieMethane(ktco2)'].values[0]
...
if __name__ == '__main__':
	...
	print(country.getMethaneValue('QA', 2012))   
